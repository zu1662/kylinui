...
- `_doc/<component>.md` 采用统一的组件说明结构：
  - 标题后先用一个或多个自然段说明组件用途、能力边界和适用场景；不得使用 Vant 等第三方组件库作为 API 来源、能力对照或实现依据，也不得保留竞品迁移式描述。
  - 每个公共组件必须包含 `## API`；复合组件在该章节下使用三级标题分别记录各子组件 Props。
  - 存在公共事件时必须包含 `## 事件`，使用表格记录事件名、触发条件和完整载荷，不得只用逗号分隔的事件名列表。
  - 存在公共插槽时必须包含 `## 插槽`，使用表格记录插槽名、用途和适用的插槽参数；默认插槽也必须列出。
  - 不存在公共事件或插槽时省略对应章节，不创建“无”或空表格。
  - 服务函数、组合式 API、实例方法、类型结构、无障碍或行为说明放在 API、事件、插槽之后的独立章节。
  - 导入及基础示例按组件复杂度补充，不重复配置实验台的全部状态。
- API 表格必须与源码保持一致，记录真实属性名、类型、默认值和说明；文档、示例和实际导出名称必须一致。
