<template>
  <label class="ky-switch" :class="[`ky-switch--${size}`, { 'is-checked': modelValue, 'is-disabled': disabled }]">
    <span class="ky-switch__label"><slot>{{ label }}</slot></span>
    <input type="checkbox" role="switch" :checked="modelValue" :disabled="disabled" :aria-label="label" @change="update" />
